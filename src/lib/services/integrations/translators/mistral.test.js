...
import mistralTranslator, { availability } from './mistral.js';
...
describe('Mistral AI Translator Service', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('Service Configuration', () => {
    it('should have correct service metadata', () => {
      expect(mistralTranslator.serviceId).toBe('mistral');
      expect(mistralTranslator.serviceLabel).toBe('Mistral');
      expect(mistralTranslator.apiLabel).toBe('Mistral AI API');
      expect(mistralTranslator.developerURL).toBe('https://docs.mistral.ai/');
      expect(mistralTranslator.apiKeyURL).toBe(
        'https://console.mistral.ai/home?profile_dialog=api-keys',
      );
      expect(mistralTranslator.apiKeyPattern).toBeInstanceOf(RegExp);
    });

    it('should validate API key pattern', () => {
      const validApiKey = 'abcdefghijklmnopqrstuvwxyz123456';
      const invalidApiKeys = ['short', '', 'too-short-key'];

      expect(mistralTranslator.apiKeyPattern.test(validApiKey)).toBe(true);

      invalidApiKeys.forEach((key) => {
        expect(mistralTranslator.apiKeyPattern.test(key)).toBe(false);
      });
    });

    it('should support markdown', () => {
      expect(mistralTranslator.markdownSupported).toBe(true);
    });
  });
  ...
  describe('Translation Function', () => {
    const mockApiKey = 'abcdefghijklmnopqrstuvwxyz123456';
    ...
    it('should successfully translate text', async () => {
      ...
      const result = await mistralTranslator.translate(texts, mockOptions);
    ...
    it('should send the correct model and reasoning_effort in the request body', async () => {
      const mockResponse = {
      ...
      await mistralTranslator.translate(['Hello world'], mockOptions);
    ...
    it('should include proper system and user messages in request', async () => {
      ...
      await mistralTranslator.translate(texts, options);
    ...
    it('should throw error for unsupported source locale', async () => {
      ...
      await expect(mistralTranslator.translate(['Hello'], invalidOptions)).rejects.toThrow(
