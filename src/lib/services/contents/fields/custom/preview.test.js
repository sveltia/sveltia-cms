...
describe('contents/fields/custom/preview-helpers', () => {
  ...
  describe('preview data caching', () => {
    /** @type {any} */
    const draft = {
      originalEntry: {
        slug: 'hello',
        locales: { en: { content: { title: 'Hello' }, path: 'a.md' } },
      },
      collectionName: 'posts',
      currentValues: { en: { title: 'Hello' } },
    };

    it('computes preview data once for several fields in the same microtask', async () => {
      const { buildPreviewData } = await import('$lib/services/api/helpers');

      vi.mocked(buildPreviewData).mockClear();

      // Two custom previews rendering in response to the same change
      buildPreviewProps({
        preview: vi.fn(),
        currentValue: 'a',
        fieldConfig: { widget: 'custom', name: 'one' },
        locale: 'en',
        draft,
      });
      buildPreviewProps({
        preview: vi.fn(),
        currentValue: 'b',
        fieldConfig: { widget: 'custom', name: 'two' },
        locale: 'en',
        draft,
      });

      expect(buildPreviewData).toHaveBeenCalledTimes(1);
    });

    it('recomputes preview data after the microtask, so values never go stale', async () => {
      const { buildPreviewData } = await import('$lib/services/api/helpers');
    ...
    it('does not share preview data between locales', async () => {
      const { buildPreviewData } = await import('$lib/services/api/helpers');
