...
- [Gatsby plugin](https://github.com/decaporg/gatsby-plugin-decap-cms): As of 2025, Gatsby is no longer a popular framework, so we won’t invest time in developing a plugin for it. You can still create `index.html` yourself. We don’t support Netlify Identity Widget. The favicon can be specified with the `logo_url` option. The `robots` meta tag is included by default with Sveltia CMS.
